...
export default function StatCard({ label, value, icon, color, change, trend }: StatCardProps) {
    const getIcon = () => {
        switch (icon) {
            case 'trending-up':
                return <TrendingUp size={32} className="text-green-400" />;
            case 'trending-down':
                return <TrendingDown size={32} className="text-red-400" />;
            case 'wallet':
                return <Wallet size={32} className="text-indigo-400" />;
            case 'bar-chart-3':
                return <BarChart3 size={32} className="text-blue-400" />;
            default:
                return null;
        }
    };
